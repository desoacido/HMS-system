...
/* 2. AGE & GENDER VALIDATION */
$birthdate = new DateTime($patient['birthdate']);
$today     = new DateTime();
$age_years = $today->diff($birthdate)->y;
$gender    = $patient['gender'];

// Ayon sa standard protocol, 13 pataas ang inaallow sa FP referral
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        // Simulan ang MySQLi Transaction
        ...
        $stmt1->bind_param("ii", $patient_id, $user_id);
        $stmt1->execute();
        ...
        $stmt2 = $conn->prepare("
            INSERT INTO family_planning_visits (
                visit_id, patient_id, blood_pressure, weight,
        ...
        $age_youngest       = $_POST['age_youngest_child'] ?? null;
        $is_breastfeeding   = $_POST['is_breastfeeding'] ?? 'No';
        $is_pregnant        = $_POST['is_pregnant'] ?? 'No';
        $is_smoker          = $_POST['is_smoker'] ?? 'No';
        $has_hypertension   = $_POST['has_hypertension'] ?? 'No';
        $has_diabetes       = $_POST['has_diabetes'] ?? 'No';
        $has_blood_clots    = $_POST['has_blood_clots'] ?? 'No';
        $has_migraines      = $_POST['has_migraines'] ?? 'No';
        $planning_pregnancy = $_POST['planning_pregnancy'] ?? 'No';
        $months_on_method   = $_POST['months_on_method'] ?? null;
        $menstrual_reg      = $_POST['menstrual_regularity'] ?? 'Regular';

        // Bind parameters (i=int, s=string, d=double/decimal)
        $stmt2->bind_param("iiddssissssssssssis", 
            $visit_id, $patient_id, $bp, $weight, 
            $method, $lmp, $num_children, $side_effects,
            $age_youngest, $is_breastfeeding, $is_pregnant,
            $is_smoker, $has_hypertension, $has_diabetes,
            $has_blood_clots, $has_migraines,
            $planning_pregnancy, $months_on_method, $menstrual_reg
        );
        $stmt2->execute();
        $fp_id = $conn->insert_id;

        // STEP 3 — INSERT INTO referrals (Para makita ni Nurse)
        $stmt3 = $conn->prepare("
            INSERT INTO referrals (
                patient_id, visit_id, source_type, source_id, referred_by, status, created_at, ml_result
            ) VALUES (?, ?, 'family_planning', ?, ?, 'pending', NOW(), 'For Review')
        ");
        $stmt3->bind_param("iiii", $patient_id, $visit_id, $fp_id, $user_id);
        $stmt3->execute();
...
<!-- HTML part remains the same as your clean UI design -->
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; font-family:'Poppins',sans-serif; }
        ...
        .form-group input, .form-group select, .form-group textarea { width:100%; padding:10px 12px; border:1px solid #ddd; border-radius:8px; font-size:13px; }
...
<form method="POST">
    ...
    <div class="form-card">
        ...
        <div class="form-group">
            ...
            <textarea name="side_effects" rows="2" placeholder="Describe side effects..."></textarea>
        </div>
    </div>
    ...
    <div class="form-card">
        ...
        <div class="screening-grid">
            ...
            <div class="screening-item">
                <span><?= $label ?></span>
                <div class="toggle">
                    <input type="radio" name="<?= $name ?>" id="<?= $name ?>_y" value="Yes"><label for="<?= $name ?>_y" class="yes">Yes</label>
                    <input type="radio" name="<?= $name ?>" id="<?= $name ?>_n" value="No" checked><label for="<?= $name ?>_n" class="no">No</label>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
